...
class ChatController extends Controller
{
    ...
    public function sendMessage(Request $request)
    {
        ...
        return response()->json(['message'=>$message, 'Email response'=>'Email queued successfully.'], 200);
    }
...
public function getRecentContacts($senderID) {
    $contactList = DB::table('messages')
                    ->select(DB::raw('CASE WHEN `from` = "'.$senderID.'" THEN `to` ELSE `from` END AS contact, MAX(created_at) as last_contact_time'))
                    ->where('from', $senderID)
                    ->orWhere('to', $senderID)
                    ->groupBy('contact')
